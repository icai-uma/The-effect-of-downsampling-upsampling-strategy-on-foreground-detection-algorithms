...
for NdxVideo=1:NumVideos
    ...
    frames = zeros(numFrames{NdxVideo}, InputSize{NdxVideo}(1), InputSize{NdxVideo}(2),3,'uint8');
    for NdxFrame=1:numFrames{NdxVideo}
        VideoFrame=imread(sprintf(strcat(params.VideoPath,'/input/in%06d.jpg'),NdxFrame));
        frames(NdxFrame,:,:,:) = VideoFrame;
    end
    save(strcat(params.VideoPath,'/frames.mat'), 'frames','-v7.3');
    clearvars frames;
end
