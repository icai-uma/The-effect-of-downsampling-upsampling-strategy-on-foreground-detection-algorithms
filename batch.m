...
TypeDownsampling = [1 2 3 4 5]; % {imresize ('nearest', 'bicubic', 'bilinear'), imfilter, SR}
...
%NdxVideo=mod(ArrayIDNumber,NumVideos)+1;
%%%[NdxVideo,NdxMethod,NdxTypeCompression,NdxCompressionFactor]=ind2sub([NumVideos NumMethods NumTypeCompressions NumCompressionFactors],ArrayIDNumber);
[NdxVideo,NdxMethod,NdxResizeFactor,NdxTypeDownsampling,NdxTypeUpsampling]=ind2sub([NumVideos NumMethods NumResizeFactors NumTypeDownsamplings NumTypeUpsamplings],ArrayIDNumber);
...
params.VideoFileSpec = strcat(params.MatlabProjectsDirectory,'dataset2014/dataset/',VideoFileSpec{NdxVideo},'/input/in%06d.jpg');
params.PathDatasetGT = strcat(params.MatlabProjectsDirectory,'dataset2014/dataset/',VideoFileSpec{NdxVideo},'/groundtruth/gt%06d.png');
params.PathDatasetVideoROI = strcat(params.MatlabProjectsDirectory,'dataset2014/dataset/',VideoFileSpec{NdxVideo},'/temporalROI.txt');
...
params.TypeUpsampling = TypeDownsampling(NdxTypeUpsampling);
...
if exist(statsFile) == 2 %file exists
    ...
    %  open matlabs to size the memory properly and close this matlab
    %[status,cmdout] = dos('matlab -r magic(8) &'); 
    commandDownsampling = strcat('matlab -r downsamplingVideo(''' , params.pathConfigFile, ''') &');
    commandUpsampling = strcat('matlab -r upsamplingSegmentation(''' , params.pathConfigFile, ''') &');    
    commandStats = strcat('matlab -r statsConfiguration(''' , params.pathConfigFile, ''') &');
    
    if isunix
        %commandDownsampling = strcat('matlab -nosoftwareopengl -r downsamplingVideo(''' , params.pathConfigFile, ''') &');
        path1 = getenv('PATH');
        path1 = [path1 ':/usr/local/MATLAB/R2018a/bin'];
        setenv('PATH', path1);
        system('sh /home/scripts/Something.sh')
        [status,cmdout] = unix(commandDownsampling);
        [status,cmdout] = unix(commandMethod);
        [status,cmdout] = unix(commandUpsampling);
        [status,cmdout] = unix(commandStats);
    elseif ispc
        [status,cmdout] = dos(commandDownsampling);
        [status,cmdout] = dos(commandMethod);
        [status,cmdout] = dos(commandUpsampling);
        [status,cmdout] = dos(commandStats);
    end
