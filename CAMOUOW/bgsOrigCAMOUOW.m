...
%%%%%%load(strcat(params.PathDatasetResized,'..','/frames.mat')); % load video frames
    ...
    videoPath = strcat(params.VideoPath,'');
	...
	command = sprintf('%s %s "" .png 0 %s %s %d', executable, videoPath, NameMethod{NdxMethod}, outputPath, numFrames{NdxVideo});
    command
    tic;
    ...
    save(strcat(params.MatlabProjectsDirectory,'resizeStudyCAMOUOW/',sprintf('segmentedFrames_%d_%d_%d_%d_%d.mat',NdxVideo,NdxMethod,NdxResizeFactor,NdxTypeDownsampling,NdxTypeUpsampling)), 'segmentedFrames','-v7.3');
