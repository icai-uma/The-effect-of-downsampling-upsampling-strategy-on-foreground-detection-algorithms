function []=checkUpsampledFiles(selectedMethod)
...
fprintf('Checking upsampling (%s)...\n', selectedMethod);
contFailedUpsampling = 0;
for NdxTypeUpsampling=1:2
    for NdxResizeFactor = 1:NumResizeFactors
        for NdxTypeDownsampling = 1:NumTypeDownsamplings
            for NdxVideo=1:NumVideos
                pathDatasetUpsampled = strcat(params.MatlabProjectsDirectory,'resizeStudy/methods/',sprintf('%d-%d-%d-%d/',NdxMethod,NdxResizeFactor,NdxTypeDownsampling,NdxTypeUpsampling),'upsampling/',VideoFileSpec{NdxVideo},'/upsampledFrames.mat');
                if exist(pathDatasetUpsampled,'file') ~= 2 %file does not exist
                    contFailedUpsampling = contFailedUpsampling + 1;
                    fprintf('%d-%d-%d-%d-%d-%s\n',NdxVideo,NdxMethod,NdxResizeFactor,NdxTypeDownsampling,NdxTypeUpsampling,VideoFileSpec{NdxVideo});
                    
                else
                    s=dir(pathDatasetUpsampled);
...
fprintf('Total failed upsampling(%s): %d\n', selectedMethod,contFailedUpsampling);
